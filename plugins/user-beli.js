...
let handler = async (m, { sock, command, text, reply, isOwner, jid, sender }) => {
    ...
    if (!global.db.users[sender]) {
        global.db.users[sender] = {
            name: m.pushName || 'Customer Neromatic',
            txHistory: []
        };
        saveDB(global.db);
    }
    ...
    switch (command) {
        ...
        case 'save_notify':
            ...
            if (!global.db.notify.includes(sender)) {
                ...
                await sock.sendMessage(ownerJid, { text: `Notifikasi Aktif: Pelanggan @${sender.split('@')[0]} minta diinfokan saat toko buka.\n*Note:* Otomatis di infokan saat kamu mengetik .mode buka`, mentions: [sender] }).catch(() => null);
        ...
        case 'konfirmasi_tf':
            ...
            if (proofMsg) {
                await sock.sendMessage(ownerJid, { forward: proofMsg }).catch(e => console.error('Gagal forward gambar:', e));
            }

            await sock.sendButton(ownerJid, {
                ...
                footer: "Owner Panel - " + (global.toko?.nama || "Neromatic"),
                buttons: [
        ...
        case 'acc':
            ...
            if (p.type === 'static') {
                ...
                if (isGroupTx) {
                    await sock.sendMessage(aOrder.jid, { text: `Haii *${targetUser.name}*, pembayaran invoice *${aOrderId}* diterima! Detail produk sudah dikirim ke Chat Pribadi (PC).` }, { quoted: m });
                    return sock.sendMessage(aOrder.userid, { text: msgToUser });
                } else {
                    return sock.sendMessage(aOrder.userid, { text: msgToUser });
                }
            }
        ...
        case 'reject_reason':
            ...
            return sock.sendMessage(rOrder.jid, { 
        ...
        case 'batal':
            ...
            if (cancelOrder.msgKey) {
                await sock.sendMessage(jid, { delete: cancelOrder.msgKey }).catch(() => null);
            }
            break;
    }
};
...
async function generateInvoice(sock, jid, sender, cProduct, cVariant, cProdId, varIdx, userData, m) {
    ...
    let qrisUrl = global.payment?.qris || 'https://via.placeholder.com/400x400.png?text=QRIS+Payment';

    let msgRes = await sock.sendButton(jid, {
        caption: invMsg,
        image: { url: qrisUrl },
        footer: global.toko?.footer || "Neromatic Payment",
        buttons: [
    ...
    setTimeout(async () => {
        ...
        if (checkOrder && checkOrder.status === 'PENDING') {
            ...
            saveDB(global.db);
            await sock.sendMessage(jid, { text: `Haii *${userData.name}*, waktu pembayaran untuk invoice *${orderId}* telah habis.` }, { quoted: m });
            if (checkOrder.msgKey) {
                await sock.sendMessage(jid, { delete: checkOrder.msgKey }).catch(() => null);
            }
        }
    }, 15 * 60 * 1000);
}
